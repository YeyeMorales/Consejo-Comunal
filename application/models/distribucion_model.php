...
class Distribucion_geopolitica_model extends CI_Model {
    ...
    function getmunicipios($id) {
        ...
        $this->db->where( array('n_estados_id' => $id) );
        return $this->db->select('n_municipios.id, n_municipios.nombre_municipio')
        ->from('n_estados')
        ->join('n_municipios', 'n_municipios.n_estados_id = n_estados.id')
        ->get()->result();
    }
    ...
    function getparroquias($id) {
        ...
        ->from('n_municipios')
        ->join('n_parroquias', 'n_parroquias.n_municipios_id = n_municipios.id')
        ->get()->result();
    }
